...
let mouseY = 0;
let connectedCollisionBlocks = []; // Blocos de colisão conectados
...
function drawMap() {
  ...
  // Desenhar os blocos de colisão conectados com sombra
  connectedCollisionBlocks.forEach(collision => {
    if (mouseOverCollision(collision.x, collision.y)) {
      ctx.save();
      ctx.globalAlpha = 0.5; // Aplicar sombra
      ctx.fillStyle = 'yellow'; // Cor de destaque
      ctx.fillRect(
        (collision.x * tileSize + offsetX) * scale, 
        (collision.y * tileSize + offsetY) * scale, 
        tileSize * scale, 
        tileSize * scale
      );
      ctx.restore();
...
function highlightCollisionBlocks(tileX, tileY) {
  connectedCollisionBlocks = [];
  const visited = {};
  addCollisionBlock(tileX, tileY, visited);
  drawMap(); // Redesenhar o mapa com os blocos destacados
}
...
function addCollisionBlock(tileX, tileY, visited) {
  ...
  visited[index] = true;
  connectedCollisionBlocks.push({ x: tileX, y: tileY });
